...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_eval_task=None, max_num_instances_per_prefix=None, **kwargs):
        ...
        self.max_num_instances_per_prefix: int = max_num_instances_per_prefix
...
class NaturalInstructions(datasets.GeneratorBasedBuilder):
        ...
        return [
            datasets.SplitGenerator(
                ...
                gen_kwargs={
                    ...
                    "max_num_instances_per_prefix": self.config.max_num_instances_per_prefix,
                    "subset": "train"
                }),
            datasets.SplitGenerator(
                name=datasets.NamedSplit("meta"),
                gen_kwargs={
                    "path": os.path.join(split_dir, "meta_tasks.txt"), 
                    "task_dir": task_dir, 
                    "max_num_instances_per_task": self.config.max_num_instances_per_task,
                    "max_num_instances_per_prefix": self.config.max_num_instances_per_prefix,
                    "subset": "meta"
    ...
    def _generate_examples(self, path=None, task_dir=None, max_num_instances_per_task=None, max_num_instances_per_prefix=None, subset=None):
        """Yields examples."""
        ...
        with open(path, encoding="utf-8") as split_f:
            for task_id, line in enumerate(split_f):
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if max_num_instances_per_prefix is not None:
                        if len(all_instances) < max_num_instances_per_prefix:
                            all_instances = all_instances * math.ceil(max_num_instances_per_prefix / len(all_instances))
                        other_task_exemplar = {"id": "", "input": "", "output": [""]}
                        other_task_exemplars = [other_task_exemplar] * (max_num_instances_per_prefix - 1)
                        task_exemplars = random.sample(all_instances, 1)
                        exemplars = other_task_exemplars + task_exemplars
                        assert len(exemplars) == max_num_instances_per_prefix
                    else:
                        exemplars = None
                        task_exemplars = None
                    ...
                    for idx, instance in enumerate(instances):
                        ...
                        if exemplars is None: exemplars = [instance]
                        example["Exemplar"] = exemplars
                        
                        if max_num_instances_per_prefix is not None:
                            if len(all_instances) < max_num_instances_per_prefix:
                                all_instances = all_instances * math.ceil(max_num_instances_per_prefix / len(all_instances))
                            example["Instance Exemplar"] = random.sample(all_instances, max_num_instances_per_prefix)
                        else:
                            example["Instance Exemplar"] = [instance]
